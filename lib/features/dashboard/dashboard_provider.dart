...
class DashboardData {
  ...
  DashboardData({
    ...
    this.productivityInsight = 'Record your first meeting to get AI-driven performance insights.',
    this.isLoading = true,
  });
...
class DashboardNotifier extends StateNotifier<DashboardData> {
  ...
  Future<void> _calculateStats() async {
    ...
    final meetings = await _ref.read(meetingRepositoryProvider).getAllMeetings();
    final tasks = await _ref.read(taskRepositoryProvider).getAllTasks();
    
    int totalMeetings = meetings.length;
    double totalSeconds = meetings.fold(0.0, (sum, m) => sum + m.durationSeconds);
    ...
    String insight;
    if (totalMeetings == 0) {
      insight = 'Start recording your meetings. MeetingMind AI will track tasks and summarize them automatically.';
    } else if (tasks.isEmpty) {
      insight = 'You have no action items yet. Keep speaking, and I will extract them from your meetings!';
    } else {
      double completionRate = completed / tasks.length;
      if (completionRate >= 0.8) {
        insight = 'Outstanding! You completed ${(completionRate * 100).toStringAsFixed(0)}% of tasks. Focus remains extremely high this week.';
      } else if (completionRate >= 0.5) {
        insight = 'On track. You completed ${(completionRate * 100).toStringAsFixed(0)}% of action items. $pending tasks are still awaiting follow-up.';
      } else {
        insight = 'Action items are piling up. Consider reviewing deadlines on pending tasks to clear bottlenecks.';
      }
    }
...
class EmotionTestNotifier extends StateNotifier<EmotionTestState> {
  ...
  Future<void> runTest() async {
    ...
    try {
      ...
      state = EmotionTestState(
        isLoading: false,
        errorMessage: 'Connection failed. Ensure the emotion backend Flask service is running.',
      );
    }
  }
...
final emotionTestProvider = StateNotifierProvider<EmotionTestNotifier, EmotionTestState>((ref) {
